...
def default_dataset() -> Dict[str, List[dict]]:
    scenario = Scenario(
        ...
        phases=[
            Phase(id=new_id("phase"), name="Früherkennung", start=0, end=25, description="Monitoring und erste Signale"),
            Phase(id=new_id("phase"), name="Belastung", start=25, end=55, description="Abweichungen und Druck auf das System"),
            Phase(id=new_id("phase"), name="Stabilisierung", start=55, end=80, description="Aktive Gegensteuerung"),
            Phase(id=new_id("phase"), name="Resilienz", start=80, end=100, description="Nachsteuerung und Lernphase"),
        ],
        events=[],
        measures=[],
    )
    ...
    scenario.events = [
        ...
        TimelineEvent(
            id=new_id("event"),
            title="Digitale Verbrauchsprognose eingeführt",
            phase_id=scenario.phases[2].id,
            position=61,
            description="Bessere Vorausschau auf Lastspitzen.",
            impact_score=5.0,
            direction="positive",
            risk_score=2.0,
            strategy_effect=4.0,
            metrics={"Prognosegüte": 14},
        ),
    ]
    scenario.measures = [
        Measure(
...
def get_scenarios() -> List[Scenario]:
    ...
    for item in raw:
        phases = [Phase(**p) for p in item.get("phases", [])]
        events = [
            TimelineEvent(
                id=e["id"],
                title=e["title"],
        ...
        scenarios.append(
            Scenario(
                id=item["id"],
                name=item["name"],
                vision_goal=item["vision_goal"],
                baseline_accuracy=float(item.get("baseline_accuracy", 50)),
                target_value=float(item.get("target_value", 100)),
...
def calculate_accuracy(scenario: Scenario) -> float:
    negative_pressure = sum(max(0.0, e.impact_score) for e in scenario.events if e.direction == "negative")
    positive_support = sum(max(0.0, e.impact_score) for e in scenario.events if e.direction == "positive")
    measure_support = sum(max(0.0, m.effectiveness_score) for m in scenario.measures if m.status in {"active", "completed"})

    score = scenario.baseline_accuracy - negative_pressure * 1.2 + positive_support * 0.8 + measure_support * 1.1
...
def wrap_text(text: str, width: int = 26) -> str:
    ...
    current = 0
    for word in words:
        additional = len(word) + (1 if line else 0)
        if current + additional <= width:
            line.append(word)
            current += additional
        else:
...
def event_y(event: TimelineEvent, sensitivity: float = 1.0) -> float:
    intensity = abs(event.strategy_effect) * 0.16 + event.impact_score * 0.11 + event.risk_score * 0.08
    direction_boost = 0.35 if event.direction == "negative" else 0.15 if event.direction == "positive" else 0.05
    return 1.15 + min(3.1, (intensity + direction_boost) * max(0.75, sensitivity))


def measure_y(measure: Measure, sensitivity: float = 1.0) -> float:
    intensity = abs(measure.strategy_alignment) * 0.16 + measure.effectiveness_score * 0.12
    status_boost = 0.3 if measure.status == "active" else 0.18 if measure.status == "completed" else 0.08
    return -1.15 - min(3.1, (intensity + status_boost) * max(0.75, sensitivity))
...
def strategy_series(scenario: Scenario) -> tuple[list[float], list[float], list[float], list[float]]:
    ...
    for pos, kind, obj in timeline_items:
        ...
        if kind == "event":
            ...
            if event.direction == "negative":
                accuracy_running -= event.impact_score * 1.15 + event.risk_score * 0.8
...
def build_timeline_figure(scenario: Scenario) -> go.Figure:
    ...
    phase_fill = ["rgba(99, 102, 241, 0.07)", "rgba(14, 165, 233, 0.07)"]
    for idx, phase in enumerate(sorted(scenario.phases, key=lambda p: p.start)):
        fig.add_vrect(
            ...
            annotation_font=dict(size=12, color="#334155"),
            fillcolor=phase_fill[idx % 2],
            opacity=1,
            line_width=0,
            layer="below",
        )
    ...
    fig.add_annotation(
        x=100, y=target_y + 0.06, text=f"Zielwert {scenario.target_value:.1f}",
            ...
            go.Scatter(
                ...
                customdata=[
                    [
                        ...
                        e.risk_score,
                        e.strategy_effect,
                        ", ".join([f"{k}: {v}" for k, v in e.metrics.items()]) or "keine Kennzahlen",
                    ]
                ...
                hovertemplate=(
                    ...
                    "Direkter Strategiebeitrag: %{customdata[4]}<br>Kennzahlen: %{customdata[5]}<extra></extra>"
                ),
                name="Szenarien / Risiken",
            )
        )
    ...
    if scenario.measures:
            ...
            go.Scatter(
                ...
                customdata=[
                    [
                        ...
                        m.strategy_alignment,
                        ", ".join([f"{k}: {v}" for k, v in m.metrics.items()]) or "keine Kennzahlen",
                    ]
        ...
        for m in scenario.measures:
            ...
            fig.add_annotation(
                x=m.position,
                y=(bottom_y + anchor_y) / 2,
                text=f"Δ {m.strategy_alignment:+.1f}",
                showarrow=False,
                xshift=18,
    ...
    fig.add_trace(
        go.Scatter(
            ...
            y=accuracy_y,
            mode="lines+markers",
            name="Szenarioplanungsgenauigkeit",
    ...
    fig.add_annotation(x=1, y=3.95, text="Oberhalb: Szenarien, Risiken und direkte Einwirkung auf die Strategie", showarrow=False, xanchor="left", font=dict(size=12, color="#334155"))
    fig.add_annotation(x=1, y=-4.05, text="Unterhalb: Maßnahmen und ihr strategischer Ausgleich", showarrow=False, xanchor="left", font=dict(size=12, color="#334155"))
    ...
    fig.update_layout(
        ...
        yaxis=dict(title="Auslenkung / Strategieverlauf", range=[-4.2, 4.2], showgrid=True, gridcolor="rgba(148,163,184,0.12)", zeroline=False, showticklabels=False),
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="left", x=0, bgcolor="rgba(255,255,255,0.9)"),
        title=f"Szenario-Zeitstrahl: {scenario.name}",
...
def main() -> None:
    ...
    st.markdown(
        """
        <style>
        .stApp {background: linear-gradient(180deg, #ffffff 0%, #f8fbff 100%); color: #0f172a;}
    ...
    st.caption("Interaktive Eingabemaske und Zeitstrahl-Visualisierung für Szenariomanagement, Abweichungen und Gegenmaßnahmen.")
    ...
    with col_left:
            ...
            with st.form("new_phase_form", clear_on_submit=True):
                ...
                if st.form_submit_button("Phase anlegen") and phase_name.strip():
                    scenario.phases.append(
                        Phase(id=new_id("phase"), name=phase_name.strip(), start=float(phase_start), end=float(phase_end), description=phase_desc.strip())
                    )
                    scenario.phases = sorted(scenario.phases, key=lambda p: p.start)
                    persist_scenarios(scenarios)
                    st.rerun()
        ...
        st.markdown("---")
        st.subheader("Ereignisse oberhalb des Zeitstrahls")
        with st.form("new_event_form", clear_on_submit=True):
            title = st.text_input("Titel des Ereignisses")
            ...
            strategy_effect = st.slider("Direkter Effekt auf die aktuelle Strategie", -10.0, 10.0, 0.0, 0.5)
            direction = st.selectbox("Richtung", ["negative", "positive", "neutral"])
            metrics_text = st.text_area("Kennzahlen (eine pro Zeile: Name: Wert)")
            if st.form_submit_button("Ereignis hinzufügen") and title.strip():
        ...
        st.subheader("Gegenmaßnahmen unterhalb des Zeitstrahls")
        with st.form("new_measure_form", clear_on_submit=True):
            title = st.text_input("Titel der Gegenmaßnahme")
            phase_id = st.selectbox("Phase für Maßnahme", options=[p.id for p in scenario.phases], format_func=lambda x: phase_name_map(scenario).get(x, x))
            min_pos, max_pos = phase_position_bounds(scenario, phase_id)
    ...
    with col_right:
        st.subheader("Analyse und Visualisierung")
        ...
        fig = build_timeline_figure(scenario)
        st.plotly_chart(fig, width="stretch")

        st.markdown("### Aktive Elemente")
        tabs = st.tabs(["Ereignisse", "Gegenmaßnahmen", "Datenexport"])
                ...
                measure_df = pd.DataFrame([
                    {
                        ...
                        "Kennzahlen": ", ".join([f"{k}: {v}" for k, v in m.metrics.items()]),
                    }
        ...
        with tabs[2]:
            ...
            st.download_button(
                "Aktives Szenario als JSON exportieren",
                data=json.dumps(export_payload, indent=2, ensure_ascii=False),
                file_name=f"{scenario.name.lower().replace(' ', '_')}.json",
            ...
            if uploaded is not None:
                try:
                    ...
                    imported = Scenario(
                        ...
                        name=payload["name"],
                        vision_goal=payload["vision_goal"],
                        ...
                        phases=[Phase(**p) for p in payload.get("phases", [])],
                        events=[
                            TimelineEvent(
                                id=e["id"],
                                title=e["title"],
                    ...
                    persist_scenarios(scenarios)
                    st.success("Szenario importiert. Bitte Seite neu laden oder im Seitenmenü auswählen.")
                except Exception as ex:
                    st.error(f"Import fehlgeschlagen: {ex}")

    st.markdown("---")
    st.markdown(
        "**Fachlogik des Prototyps**: Oberhalb des Zeitstrahls werden externe und interne Entwicklungen visualisiert, "
        "die die Abweichung vom visionsstrategischen Ziel beeinflussen. Unterhalb werden geplante, aktive und bereits wirksame "
        "Gegenmaßnahmen abgebildet. Die Szenarioplanungsgenauigkeit wird als verdichtete Verlaufskurve dargestellt."
    )
