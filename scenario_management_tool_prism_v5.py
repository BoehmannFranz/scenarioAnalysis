...
def build_prism_figure(
    ...
    for i, lane in enumerate(lane_keys):
        r = profiles[lane]
        X[i, :] = z
        Y[i, :] = r * np.cos(angles[i])
        Z[i, :] = r * np.sin(angles[i])
    X[n, :] = X[0, :]
    Y[n, :] = Y[0, :]
    Z[n, :] = Z[0, :]
    ...
    for e in scenario.events:
        for lane in e.lanes:
            ...
            angle = angles[i]
            x = e.position
            y = (r + 0.03) * math.cos(angle)
            z0 = (r + 0.03) * math.sin(angle)
    ...
    for m in scenario.measures:
        for lane in m.lanes:
            ...
            angle = angles[i]
            x = m.position
            y = max(EPSILON_RADIUS * 0.6, r - 0.02) * math.cos(angle)
            z0 = max(EPSILON_RADIUS * 0.6, r - 0.02) * math.sin(angle)
            fig.add_trace(go.Scatter3d(
                x=[x], y=[y], z=[z0], mode="markers+text",
...
def main() -> None:
    ...
    with right:
        ...
        rotation_deg = st.slider("Rotation um die Zeitachse", -180, 180, 0, 5)
        section_week = st.slider("Querschnitt / Zeitpunkt", 0.0, 100.0, 50.0, 1.0)

        t1, t2, t3 = st.tabs(["3D-Körper", "Ausgerollte Fläche", "Querschnitt"])
        with t1:
            st.caption("Die Zeitachse bleibt fest horizontal. Die Ansicht rotiert nur um diese Achse.")
            st.plotly_chart(
                build_prism_figure(scenario, highlighted_lanes, rotation_deg=rotation_deg),
                width="stretch",
    ...
    st.markdown(
        ...
        "Der Speichern-Button überschreibt `scenario_data.json` und zeigt den Status an. Die 3D-Ansicht rotiert kontrolliert nur um die feste horizontale Zeitachse."
    )
