...
def default_dataset() -> Dict[str, List[dict]]:
    ...
    scenario = Scenario(
        ...
        notes="Visualisierung als strategischer Körper mit polygonalem Querschnitt.",
        phases=phases,
        events=[
            TimelineEvent(
                id=new_id("event"),
...
def calculate_accuracy(scenario: Scenario) -> float:
    negative_pressure = sum(max(0.0, e.impact_score + 0.7 * e.risk_score) * max(1, len(e.lanes)) * 0.6 for e in scenario.events if e.direction == "negative")
    positive_support = sum(max(0.0, e.impact_score + max(0.0, e.strategy_effect)) * max(1, len(e.lanes)) * 0.5 for e in scenario.events if e.direction == "positive")
    measure_support = sum(
...
def lane_signal(scenario: Scenario, lane: str, x: np.ndarray) -> np.ndarray:
    ...
    for e in scenario.events:
        ...
        amplitude = sign * (0.65 * e.impact_score + 0.55 * e.risk_score + 0.8 * abs(e.strategy_effect)) * lane_share
    ...
    for m in scenario.measures:
        ...
        amplitude = (0.75 * m.effectiveness_score + 0.9 * m.strategy_alignment) * STATUS_FACTOR.get(m.status, 0.5) * lane_share
...
def build_unwrapped_timeline_figure(scenario: Scenario, show_total: bool = True, selected_lanes: Optional[List[str]] = None) -> go.Figure:
    ...
    for i, phase in enumerate(sorted(scenario.phases, key=lambda p: p.start)):
        fig.add_vrect(x0=phase.start, x1=phase.end, fillcolor=phase_colors[i % 2], line_width=0, layer="below", annotation_text=phase.name, annotation_position="top left")

    lane_offsets = {}
    top = max(2.4, len(selected_lanes) * 0.72)
    step = 1.35
    ...
    if show_total:
        ...
        fig.add_trace(go.Scatter(x=x, y=(top + 1.4) + 0.25 * total, mode="lines", name="Gesamtwirkung", line=dict(width=5, dash="dash"), hovertemplate="Gesamtwirkung<br>Zeit: %{x:.1f}<br>Signal: %{y:.2f}<extra></extra>"))
        fig.add_annotation(x=101, y=(top + 1.4), text="Gesamtwirkung", showarrow=False, xanchor="left", font=dict(size=11, color="#111827"))

    for lane in selected_lanes:
        meta = STRATEGIC_LANES[lane]
        offset = lane_offsets[lane]
        signal = lane_signal(scenario, lane, x)
        y = offset + 0.18 * signal
        fig.add_trace(go.Scatter(x=x, y=y, mode="lines", name=meta["label"], line=dict(width=3), hovertemplate=f"{meta['label']}<br>Zeit: %{{x:.1f}}<br>Verlauf: %{{y:.2f}}<extra></extra>"))
        fig.add_annotation(x=101, y=offset, text=meta["label"], showarrow=False, xanchor="left", font=dict(size=11, color="#334155"))

    for e in scenario.events:
        for lane in [l for l in e.lanes if l in selected_lanes]:
            offset = lane_offsets[lane]
            local_y = offset + 0.18 * lane_signal(scenario, lane, np.array([e.position]))[0]
            fig.add_trace(go.Scatter(
                x=[e.position], y=[local_y], mode="markers+text", text=[e.title], textposition="top center",
                marker=dict(size=8 + e.risk_score, symbol="circle"), name=f"Szenario {e.title}", showlegend=False,
            ...
            fig.add_shape(type="line", x0=e.position, x1=e.position, y0=offset, y1=local_y, line=dict(width=1, dash="dot"))

    for m in scenario.measures:
        for lane in [l for l in m.lanes if l in selected_lanes]:
            offset = lane_offsets[lane]
            local_y = offset + 0.18 * lane_signal(scenario, lane, np.array([m.position]))[0]
            fig.add_trace(go.Scatter(
                x=[m.position], y=[local_y], mode="markers+text", text=[m.title], textposition="bottom center",
                marker=dict(size=8 + m.effectiveness_score, symbol="diamond"), name=f"Gegenmaßnahme {m.title}", showlegend=False,
            ...
            fig.add_shape(type="line", x0=m.position, x1=m.position, y0=offset, y1=local_y, line=dict(width=1, dash="dot"))
...
def build_prism_figure(scenario: Scenario, highlighted_lanes: Optional[List[str]] = None) -> go.Figure:
    ...
    fig = go.Figure()
    fig.add_trace(go.Surface(x=X, y=Y, z=Z, surfacecolor=np.tile(np.arange(n + 1).reshape(-1, 1), (1, len(z))), colorscale="Blues", showscale=False, opacity=0.62, hoverinfo="skip", name="Strategischer Körper"))
    ...
    for i, lane in enumerate(lane_keys):
        ...
        fig.add_trace(go.Scatter3d(
            ...
            opacity=1.0 if is_highlight else 0.28,
    ...
    for m in scenario.measures:
        for lane in m.lanes:
            ...
            fig.add_trace(go.Scatter3d(
                x=[x], y=[y], z=[m.position], mode="markers+text",
    ...
    fig.update_layout(
        ...
        scene=dict(
            ...
            aspectmode="manual",
            aspectratio=dict(x=2.15, y=1.0, z=0.9),
            camera=dict(eye=dict(x=0.0, y=2.35, z=0.08), up=dict(x=0, y=0, z=1), center=dict(x=0, y=0, z=0)),
        ),
        legend=dict(orientation="h"),
    )
    return fig
...
def build_cross_section_figure(scenario: Scenario, section_week: float, selected_lanes: Optional[List[str]] = None) -> go.Figure:
    ...
    fig.add_trace(go.Scatter(x=base_x, y=base_y, mode="lines", line=dict(width=2, dash="dash"), name="Referenzquerschnitt"))
    ...
    for x, y, lane, r in zip(xs, ys, lane_keys, radii):
        fig.add_annotation(x=x, y=y, text=f"{STRATEGIC_LANES[lane]['label']}<br>{r-6.0:+.2f}", showarrow=False, font=dict(size=11))
...
def main() -> None:
    ...
    st.caption("Linke Seite Steuerung, rechte Seite strategischer Körper: horizontal ausgerichtete 3D-Ansicht, ausgerollte Fläche und interaktiver Querschnitt.")
    ...
    with left:
        ...
        st.dataframe(pd.DataFrame([{"Key": k, "Verlauf": v["label"], "Ecke": i + 1} for i, (k, v) in enumerate(STRATEGIC_LANES.items())]), width="stretch")
    ...
    with right:
        ...
        with t1:
            st.plotly_chart(build_prism_figure(scenario, highlighted_lanes), width="stretch")
        with t2:
        ...
        st.caption("Die 3D-Ansicht startet mit horizontal ausgerichteter Zeitachse. Szenarien und Maßnahmen können mehreren strategischen Verläufen zugeordnet und selektiv hervorgehoben werden.")

    st.markdown("---")
    st.markdown(
        "**Fachlogik**: Jede Ecke des Körpers repräsentiert einen strategischen Verlauf. "
        "Szenarien können auf mehrere Verläufe gleichzeitig wirken, Gegenmaßnahmen ebenso. "
        "Die 3D-Ansicht zeigt den Körper räumlich, die ausgerollte Fläche zeigt Einzel- und Gesamtwirkungen und der Querschnitt zeigt die Systemform an einem per Slider gewählten Zeitpunkt."
    )
