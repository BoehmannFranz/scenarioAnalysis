...
def _ramp(x: np.ndarray, position: float, width: float) -> np.ndarray:
    return 1.0 / (1.0 + np.exp(-(x - position) / max(0.8, width)))


def lane_signal(scenario: Scenario, lane: str, x: np.ndarray) -> np.ndarray:
    # All lanes begin on the timeline null line and move outward over time.
    y = scenario.outward_drift * (x / 100.0)

    for e in scenario.events:
        if lane not in e.lanes:
            continue
        lane_share = 1.0 / max(1, len(e.lanes))
        amplitude = (0.75 * e.impact_score + 0.65 * e.risk_score + 0.45 * abs(e.strategy_effect)) * lane_share
        if e.direction == "positive":
            amplitude *= 0.65
        width = 2.8 + 0.22 * e.risk_score
        y += amplitude * _ramp(x, e.position, width)

    for m in scenario.measures:
        if lane not in m.lanes:
            continue
        lane_share = 1.0 / max(1, len(m.lanes))
        amplitude = (0.85 * m.effectiveness_score + 0.95 * m.strategy_alignment) * STATUS_FACTOR.get(m.status, 0.5) * lane_share
        width = 2.5
        y -= amplitude * _ramp(x, m.position, width)

    return np.maximum(0.0, y * scenario.strategy_sensitivity)
...
def build_unwrapped_timeline_figure(scenario: Scenario, show_total: bool = True, selected_lanes: Optional[List[str]] = None) -> go.Figure:
    ...
    fig.add_annotation(x=100, y=0.1, text="Nullinie / Timeline", showarrow=False, xanchor="right", font=dict(size=11, color="#334155"))
    ...
    if show_total:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                hovertemplate="Gesamtwirkung<br>Zeit: %{x:.1f}<br>Auslenkung: %{y:.2f}<extra></extra>",
            )
        )
    ...
    for lane in selected_lanes:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                line=dict(width=3),
                hovertemplate=f"{STRATEGIC_LANES[lane]['label']}<br>Zeit: %{{x:.1f}}<br>Auslenkung: %{{y:.2f}}<extra></extra>",
            )
        )
...
def build_prism_figure(scenario: Scenario, highlighted_lanes: Optional[List[str]] = None) -> go.Figure:
    ...
    profiles = {k: EPSILON_RADIUS + 0.30 * lane_signal(scenario, k, z) for k in lane_keys}
    ...
    for i, lane in enumerate(lane_keys):
        ...
        fig.add_trace(
            go.Scatter3d(
                ...
                name=STRATEGIC_LANES[lane]["label"],
                hovertemplate=f"{STRATEGIC_LANES[lane]['label']}<br>Zeit: %{{z:.1f}}<br>Radius: %{{x:.2f}}, %{{y:.2f}}<extra></extra>",
            )
        )
    ...
    for e in scenario.events:
        for lane in e.lanes:
            ...
            r = EPSILON_RADIUS + 0.30 * lane_signal(scenario, lane, np.array([e.position]))[0]
            angle = angles[i]
            x = (r + 0.05) * math.cos(angle)
            y = (r + 0.05) * math.sin(angle)
    ...
    for m in scenario.measures:
        for lane in m.lanes:
            i = lane_keys.index(lane)
            r = EPSILON_RADIUS + 0.30 * lane_signal(scenario, lane, np.array([m.position]))[0]
            angle = angles[i]
            x = max(EPSILON_RADIUS * 0.6, r - 0.04) * math.cos(angle)
            y = max(EPSILON_RADIUS * 0.6, r - 0.04) * math.sin(angle)
            fig.add_trace(go.Scatter3d(
    ...
    fig.update_layout(
        ...
        scene=dict(
            ...
            aspectmode="manual",
            aspectratio=dict(x=2.8, y=1.0, z=0.55),
            camera=dict(eye=dict(x=2.4, y=0.0, z=0.03), up=dict(x=0, y=0, z=1), center=dict(x=0, y=0, z=0)),
        ),
        legend=dict(orientation="h"),
    )
    return fig
...
def build_cross_section_figure(scenario: Scenario, section_week: float, selected_lanes: Optional[List[str]] = None) -> go.Figure:
    ...
    radii = [EPSILON_RADIUS + 0.30 * lane_signal(scenario, k, np.array([section_week]))[0] for k in lane_keys]
    ...
    base_x = [0.0 for _ in angles] + [0.0]
    base_y = [0.0 for _ in angles] + [0.0]

    fig.add_trace(go.Scatter(x=base_x, y=base_y, mode="markers", marker=dict(size=6), name="Nullzentrum"))
...
def main() -> None:
    ...
    st.caption("Alle Strategieverläufe starten auf der Nullinie. Szenarien treiben sie nach außen, Gegenmaßnahmen ziehen sie wieder zurück.")
    ...
    if sidebar.button("Alle aktuellen Daten in scenario_data.json speichern", width="stretch"):
        persist_scenarios(scenarios)
        sidebar.success("scenario_data.json wurde überschrieben.")
    ...
    st.markdown("---")
    st.markdown(
        "**Fachlogik**: Alle Strategieverläufe starten auf der Nullinie. Szenarien erhöhen die Auslenkung nach außen. "
        "Ohne Gegenmaßnahmen laufen die Verläufe weiter outward. Definierte Maßnahmen ziehen einzelne Verläufe wieder Richtung Nullinie zurück. "
        "Jeder Speichervorgang überschreibt die Datei `scenario_data.json`."
    )
