...
def build_prism_figure(
    ...
    fig.update_layout(
        ...
        scene=dict(
            ...
            aspectmode="manual",
            aspectratio=dict(x=4.6, y=0.75, z=0.75),
            dragmode=False,
            camera=dict(
                eye=dict(
                    x=0.35 / max(0.35, zoom),
                    y=(5.2 * math.cos(math.radians(rotation_deg))) / max(0.35, zoom),
                    z=(5.2 * math.sin(math.radians(rotation_deg))) / max(0.35, zoom),
                ),
                up=dict(x=1, y=0, z=0),
